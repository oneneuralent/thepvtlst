...
export async function POST(request: NextRequest) {
  ...
  if (!spreadsheet_id || !range || !values) {
    return NextResponse.json({ error: "spreadsheet_id, range, and values are required" }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to write Google Sheets data" }, { status: 500 });
  }
}
